...
$sql = "SELECT 
 ...
 WHERE wp.completed = 0 AND wp.Group2 = ${workweek} AND pcseq.AssemblyQuantity > 0 AND pci.MainPiece = 1 and wp.WorkshopID = 1";
...
foreach ($result as $row) {
    ...
    if (!isset($grouped_data[$pciseq_id])) {
        $grouped_data[$pciseq_id] = [
            ...
            'TotalNetWeight' => $row['NetAssemblyWeightEach'] * $row['MainPieceQuantity'],
            'AssemblyManHoursEach' => $row['AssemblyManHoursEach'],
            'TotalEstimatedManHours' => $row['AssemblyManHoursEach'] * $row['MainPieceQuantity'],
