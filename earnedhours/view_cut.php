...
try{
    ...
    $sql = "SELECT
        ...
        case 
            when pcis.PieceMark is NULL then 
                (select pci1.ManHours * pcis.Quantity
        ...
        where pcis.DateCompleted = :query_date and stations.Description = 'cut' and wp.WorkshopID = 1
        order by ManHours desc";
    ...
    $stmt->execute();
    $sum = 0;
    while($value = $stmt->fetchColumn(14)){
        $sum += $value;
    }
    $sum = $sum*0.2;
...
<head>
    ...
    <title>Cut Earned Man Hours Table</title>
...
<div class="container-fluid px-1 py-1">
    <div class="card border-0 shadow mb-2">
        <div class="card-header bg-gradient-primary text-white text-center py-2 px-2">
            <h1 class="h4 mb-1">Cut Details for SSF <?= $query_date ?></h1>
            <p class="small mb-0 opacity-75">Displaying <?php echo count($info).' Items and '.number_format($sum,2);?> Earned Man Hours Total</p>
        </div>
    </div>
    <div class="card border-0 shadow mb-2">
            ...
            <div class="table-container-custom border rounded shadow-sm">
                <table class="table table-striped table-hover table-sm mb-0" id="dataTable">
                    ...
                    <?php foreach($info as $row): ?>
                        <tr data-manhours="<?= round($row['ManHours']*.2,2)?>" data-jobnumber="<?= $row['JobNumber']?>">
                            ...
                            <td class="text-center py-1 px-1 fw-bold manhours-cell"><?= round($row['ManHours']*.2,2)?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
