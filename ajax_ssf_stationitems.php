...
$filterday = isset($_GET['date']) ? $_GET['date'] : date('Y-m-d');
...
LEFT JOIN users ON users.UserID = pcis.UserID
WHERE pcis.DateCompleted = :today ";

$stmt = $db->prepare($sql);
$tkdata = $stmt->execute(array('today' => $filterday));
...
foreach ($tkdata as $row) {
    ...
    $processedRow = [
        ...
        'AssemblyManHoursEach' => round($row['AssemblyManHoursEach'],2),
        ...
        'CalculatedHours' => round($stationHours[$row['StationName']] ?? 0, 2),
